				...
				<ul class="sidebar-links" id="simple-bar">
					...
					<li class="sidebar-list">
						<a class="sidebar-link sidebar-title {{request()->route()->getPrefix() == '/manager' ? 'active' : '' }}"
						   href="#"><i data-feather="home"></i><span class="lan-3">{{ __('Отдел продаж') }}</span>
						...
						<ul class="sidebar-submenu"
							style="display: {{ request()->route()->getPrefix() == '/manager' ? 'block;' : 'none;' }}">
...
{{--							@can('manage_user')--}}
								<li><a href="{{route('users')}}"
									   class="lan-4 {{ Route::currentRouteName()=='users' ? 'active' : '' }}">{{ __('Пользователи')}}</a>
								</li>
								<li><a href="{{route('user.create')}}"
									   class="lan-4 {{ Route::currentRouteName()=='user.create' ? 'active' : '' }}">{{ __('Добавить')}}</a>
								</li>
								<li><a href="{{route('roles')}}"
									   class="lan-4 {{ Route::currentRouteName()=='roles' ? 'active' : '' }}">{{ __('Роли')}}</a>
								</li>
								<li><a href="{{route('permission')}}"
									   class="lan-4 {{ Route::currentRouteName()=='permission' ? 'active' : '' }}">{{ __('Доступы')}}</a>
								</li>
{{--							@endcan--}}
						</ul>
					</li>
					<li class="sidebar-list">
						<a class="sidebar-link sidebar-title {{request()->route()->getPrefix() == '/leads' ? 'active' : '' }}"
						   href="#"><i data-feather="home"></i><span class="lan-3">{{ __('Заявки') }}</span>
							<div class="according-menu"><i
